...
import (
	...
	"github.com/mushanyux/MSIM/internal/server"
...
var (
	...
	rootCmd             = &cobra.Command{
		Use:   "ms",
		Short: "MuShanIM, a sleek and high-performance instant messaging platform.",
		Long:  `MuShanIM, a sleek and high-performance instant messaging platform. For more details, please refer to the documentation: https://githubim.com`,
	...
	if strings.TrimSpace(cfgFile) != "" {
		...
		if err := vp.ReadInConfig(); err != nil {
			if !ignoreMissingConfig {
				...
				mslog.Error("read config file error", zap.Error(err))
			}
		}
	}
...
func cmdRun() error {
	...
	if daemon { // 后台运行
		...
		err := s.Start()
		if err != nil {
			mslog.Error("start server error", zap.Error(err))
			return err
		}
		// 等待集群准备好
		...
		mslog.Info("MuShanIM server started successfully. Press Ctrl+C to exit.")

		// 阻塞直到收到退出信号
		<-quit

		s.Stop() // <-- 调用 Stop() 进行清理
		mslog.Info("MuShanIM server stopped.")
	}
	return nil
}
...
func handlePingback() error {
	...
	if err != nil {
		mslog.Error("read confirmation error from stdin", zap.Error(err))
		return err
	}
	conn, err := net.Dial("tcp", pingback)
	if err != nil {
		mslog.Error("dialing confirmation address", zap.Error(err))
		return err
	}
	defer conn.Close()
	...
	if err != nil {
		mslog.Error("write confirmation error", zap.Error(err))
		return err
	}
	return nil
}
...
func writePIDFile() error {
	...
	if err != nil {
		mslog.Error("write pid file error", zap.Error(err))
		return err
	}
	return nil
}
...
func newRunCmd(listener net.Listener) (*exec.Cmd, error) {
	...
	if !msutil.ArrayContains(newArgs, "--noStdout") {
...
func startAsChildProcess() {
	...
	if err != nil {
		mslog.Panic("listen error", zap.Error(err))
	}
	defer ln.Close()

	cmd, err := newRunCmd(ln)
	if err != nil {
		mslog.Panic("new cmd failed", zap.Error(err))
	}

	stdinPipe, err := cmd.StdinPipe()
	if err != nil {
		mslog.Panic("get stdin pipe failed", zap.Error(err))
	}
	...
	if err != nil {
		mslog.Panic("rand read error", zap.Error(err))
	}
	...
	err = cmd.Start()
	if err != nil {
		mslog.Panic("starting MuShanIM process failed", zap.Error(err))
	}
	...
	select {
	case <-success:
		mslog.Info("Successfully started MuShanIM - is running in the background", zap.Int("pid", cmd.Process.Pid))
	case err := <-exit:
		mslog.Error("MuShanIM process exited with error", zap.Error(err))
	}
}
