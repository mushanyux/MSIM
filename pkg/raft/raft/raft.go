...
import (
	...
	wt "github.com/mushanyux/MSIM/pkg/wait"
...
func (r *Raft) learnTo(learnerId uint64) (types.Config, error) {
	...
	cfg.Learners = msutil.RemoveUint64(cfg.Learners, learnerId)

	if !msutil.ArrayContainsUint64(cfg.Replicas, cfg.MigrateTo) {
		cfg.Replicas = append(cfg.Replicas, cfg.MigrateTo)
	}

	if cfg.MigrateFrom != cfg.MigrateTo {
		cfg.Replicas = msutil.RemoveUint64(cfg.Replicas, cfg.MigrateFrom)
	}
...
func (r *Raft) followerToLeader(followerId uint64) (types.Config, error) {
	cfg := r.node.Config().Clone()
	if !msutil.ArrayContainsUint64(cfg.Replicas, followerId) {
