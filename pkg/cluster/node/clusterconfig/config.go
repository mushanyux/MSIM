...
func NewConfig(opts *Options) *Config {
	cfg := &Config{
		...
		opts: opts,
		Log:  mslog.NewMSLog("cluster.config"),
	}
...
func (c *Config) loadConfigFromFile() error {
	...
	if len(data) > 0 {
		c.inited = true
		if err := msutil.ReadJSONByByte(data, c.cfg); err != nil {
...
func (c *Config) updateNodeJoining(nodeId uint64) {
	...
	c.cfg.Learners = msutil.RemoveUint64(c.cfg.Learners, nodeId)
...
func (c *Config) updateSlotMigrate(slotId uint32, fromNodeId, toNodeId uint64) {
	...
	for _, slot := range c.cfg.Slots {
		if slot.Id == slotId && slot.MigrateFrom == 0 && slot.MigrateTo == 0 {
			...
			if !msutil.ArrayContainsUint64(slot.Replicas, toNodeId) { // 如果目标节点不在副本列表里，则要加入学习者列表
...
func (c *Config) saveConfig() error {
	...
	_, err = c.cfgFile.Write([]byte(msutil.ToJSON(c.cfg)))
	if err != nil {
		return err
	}
	return nil
