...
func (s *Store) handleChannelClusterConfigSaves(reqs []*channelCfgReq) error {
	...
	cfgs := make([]msdb.ChannelClusterConfig, 0, len(reqs))
...
func (s *Store) handleBatchUpdateConversation(cmd *CMD) error {
	...
	for _, model := range models {
		var conversationType = msdb.ConversationTypeChat
		...
		for uid, seq := range model.Uids {
			...
			err = s.wdb.AddOrUpdateConversationsWithUser(uid, []msdb.Conversation{conversation})
			if err != nil {
				return err
			}
		}
...
func (s *Store) handleUpdateUserPluginNo(cmd *CMD) error {
	...
	return s.wdb.AddOrUpdatePluginUsers([]msdb.PluginUser{
		pluginUser,
	})
}
