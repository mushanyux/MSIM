...
type ICluster interface {
	...
	// Route 设置接受请求的路由
	Route(path string, handler msserver.Handler)
...
type IClusterSlot interface {
	...
	GetOrCreateChannelClusterConfigFromSlotLeader(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error)
	...
	LoadOnlyChannelClusterConfig(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error)
