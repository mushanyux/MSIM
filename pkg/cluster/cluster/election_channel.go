...
func (s *Server) getOrCreateChannelClusterConfigFromLocal(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error) {
	// 获取频道槽领导
	slotLeaderId, err := s.SlotLeaderIdOfChannel(channelId, channelType)
	if err != nil {
		return msdb.EmptyChannelClusterConfig, err
	}
	...
	if s.opts.ConfigOptions.NodeId != slotLeaderId {
		...
		return msdb.EmptyChannelClusterConfig, errors.New("not slot leader")
	}
	...
	// 如果配置不存在，则创建一个新的配置
	if msdb.IsEmptyChannelClusterConfig(cfg) {
...
// 视情况是否需要加入新的副本
func (s *Server) joinNewRepliceIfNeed(cfg *msdb.ChannelClusterConfig) (bool, error) {
	...
	for _, node := range allowVoteNodes {
		if !msutil.ArrayContainsUint64(cfg.Replicas, node.Id) {
...
// 视情况是否需要变更领导节点
func (s *Server) switchLeaderIfNeed(cfg *msdb.ChannelClusterConfig) (bool, error) {
...
// 创建一个频道的分布式配置
func (s *Server) createChannelClusterConfig(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error) {
	allowVoteNodes := s.cfgServer.AllowVoteAndJoinedOnlineNodes() // 获取允许投票的在线节点
	if len(allowVoteNodes) == 0 {
		return msdb.EmptyChannelClusterConfig, errors.New("no allow vote nodes")
	}

	createdAt := time.Now()
	updatedAt := time.Now()
	clusterConfig := msdb.ChannelClusterConfig{
		ChannelId:       channelId,
