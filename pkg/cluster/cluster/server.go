...
import (
	...
	"github.com/mushanyux/MSIM/pkg/cluster/channel"
	"github.com/mushanyux/MSIM/pkg/cluster/node/clusterconfig"
	"github.com/mushanyux/MSIM/pkg/cluster/node/event"
	"github.com/mushanyux/MSIM/pkg/cluster/node/types"
	"github.com/mushanyux/MSIM/pkg/cluster/slot"
	"github.com/mushanyux/MSIM/pkg/cluster/store"
	"github.com/mushanyux/MSIM/pkg/keylock"
...
func New(opts *Options) *Server {
	...
	s.db = msdb.NewMushanDB(
		msdb.NewOptions(
			msdb.WithShardNum(opts.DB.MSDbShardNum),
			msdb.WithDir(path.Join(opts.DataDir, "db")),
			msdb.WithNodeId(opts.ConfigOptions.NodeId),
			msdb.WithMemTableSize(opts.DB.MSDbMemTableSize),
			msdb.WithSlotCount(int(opts.ConfigOptions.SlotCount)),
		),
	)

	// 节点之间通讯的网络服务
	s.netServer = msserver.New(
		opts.Addr,
		msserver.WithMessagePoolOn(false),
		msserver.WithOnRequest(func(conn gnet.Conn, req *proto.Request) {
			trace.GlobalTrace.Metrics.System().IntranetIncomingAdd(int64(len(req.Body)))
		}),
		msserver.WithOnResponse(func(conn gnet.Conn, resp *proto.Response) {
...
func (s *Server) updateChannelCfgByConfig(cfg *msdb.ChannelClusterConfig, c rafttype.Config) {
