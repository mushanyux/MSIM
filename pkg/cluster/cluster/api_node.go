...
func (s *Server) nodesGet(c *mshttp.Context) {
	...
	for _, node := range s.cfgServer.Nodes() {
		...
		if !node.Online {
			...
			nodeCfg.IsLeader = msutil.BoolToInt(leaderId == node.Id)
...
func (s *Server) nodeGet(c *mshttp.Context) {
	nodeCfg := s.getLocalNodeInfo()
	c.JSON(http.StatusOK, nodeCfg)
}

func (s *Server) simpleNodesGet(c *mshttp.Context) {
	cfg := s.cfgServer.GetClusterConfig()
...
func (s *Server) nodeChannelsGet(c *mshttp.Context) {
	idStr := c.Param("id")
	...
	offsetCreatedAt := msutil.ParseInt64(c.Query("offset_created_at"))
	pre := msutil.ParseInt(c.Query("pre"))

	// running := msutil.ParseBool(c.Query("running")) // 是否只查询运行中的频道
	...
	channelClusterConfigs, err := s.db.SearchChannelClusterConfig(msdb.ChannelClusterConfigSearchReq{
		ChannelId:       channelId,
		ChannelType:     channelType,
		OffsetCreatedAt: offsetCreatedAt,
		Pre:             pre == 1,
		Limit:           limit + 1, // 多查一个，用于判断是否有下一页
	}, func(cfg msdb.ChannelClusterConfig) bool {
