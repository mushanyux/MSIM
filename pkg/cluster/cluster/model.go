...
import (
	...
	rafttypes "github.com/mushanyux/MSIM/pkg/raft/types"
...
// 	enc := msproto.NewEncoder()
// 	defer enc.End()
// 	enc.WriteUint8(uint8(msutil.BoolToInt(c.ClusterConfigOld)))
...
// 		return err
// 	}
// 	c.ClusterConfigOld = msutil.IntToBool(int(clusterConfigOld))
...
// 	enc := msproto.NewEncoder()
// 	defer enc.End()
// 	enc.WriteUint8(uint8(msutil.BoolToInt(s.ClusterConfigOld)))
...
// 		return err
// 	}
// 	s.ClusterConfigOld = msutil.IntToBool(int(clusterConfigOld))
...
type ChannelClusterStorage interface {
	// 保存分布式配置
	Save(clusterCfg msdb.ChannelClusterConfig) error
	// 删除频道分布式配置
	// Delete(channelId string, channelType uint8) error
	// 获取分布式配置
	Get(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error)
	...
	// // 获取某个槽位的频道分布式配置
	GetWithSlotId(slotId uint32) ([]msdb.ChannelClusterConfig, error)

	GetAll(offsetId uint64, limit int) ([]msdb.ChannelClusterConfig, error)
	// 提案配置
	Propose(cfg msdb.ChannelClusterConfig) error
	// // 获取所有槽位的频道分布式配置
	// GetWithAllSlot() ([]*msstore.ChannelClusterConfig, error)
}
...
type ChannelClusterConfigResp struct {
	...
	Status            msdb.ChannelClusterStatus `json:"status"`              // 状态
...
func NewChannelClusterConfigRespFromClusterConfig(slotLeaderId uint64, slotId uint32, cfg msdb.ChannelClusterConfig) *ChannelClusterConfigResp {

	channelTypeFormat := formatChannelType(cfg.ChannelType)

	statusFormat := ""
	if cfg.Status == msdb.ChannelClusterStatusNormal {
		statusFormat = "正常"
	} else if cfg.Status == msdb.ChannelClusterStatusCandidate {
		statusFormat = "选举中"
	} else {
	...
	if cfg.CreatedAt != nil {
		...
		createdAtFormat = msutil.ToyyyyMMddHHmmss(*cfg.CreatedAt)
	}

	updatedAt := int64(0)
	if cfg.UpdatedAt != nil {
		updatedAt = cfg.UpdatedAt.UnixNano()
		updatedAtFormat = msutil.ToyyyyMMddHHmmss(*cfg.UpdatedAt)
	}
...
func formatChannelType(channelType uint8) string {
	channelTypeFormat := ""
	switch channelType {
	case msproto.ChannelTypeGroup:
		channelTypeFormat = "群组"
	case msproto.ChannelTypePerson:
		channelTypeFormat = "个人"
	case msproto.ChannelTypeCommunity:
		channelTypeFormat = "社区"
	case msproto.ChannelTypeCustomerService:
		channelTypeFormat = "客服"
	case msproto.ChannelTypeInfo:
		channelTypeFormat = "资讯"
	case msproto.ChannelTypeData:
...
func (s *Server) requestSlotInfo(nodeId uint64, slotIds []uint32, headers map[string]string) ([]*SlotResp, error) {
	...
	resp, err := network.Get(fmt.Sprintf("%s%s", node.ApiServerAddr, s.formatPath("/slots")), map[string]string{
		"ids": strings.Join(msutil.Uint32ArrayToStringArray(slotIds), ","),
	}, headers)
	if err != nil {
		return nil, err
	}
	slotResps := make([]*SlotResp, 0)
	err = msutil.ReadJSONByByte([]byte(resp.Body), &slotResps)
...
func newMessageResp(m msdb.Message) *messageResp {

	timestampFormat := msutil.ToyyyyMMddHHmm(time.Unix(int64(m.Timestamp), 0))
...
func NewNodeConfigFromNode(n *types.Node) *NodeConfig {
	...
	lastOffline := ""
	if n.LastOffline != 0 {
		lastOffline = msutil.ToyyyyMMddHHmm(time.Unix(n.LastOffline, 0))
	}
	status := ""
	...
	return &NodeConfig{
		...
		Online:        msutil.BoolToInt(n.Online),
		OfflineCount:  int(n.OfflineCount),
		LastOffline:   lastOffline,
		AllowVote:     msutil.BoolToInt(n.AllowVote),
		Status:        n.Status,
		StatusFormat:  status,
	}
}
...
func newChannelInfoResp(ch msdb.ChannelInfo, slotId uint32) *channelInfoResp {

	lastMsgTimeFormat := ""
	if ch.LastMsgTime != 0 {
		lastMsgTimeFormat = msutil.ToyyyyMMddHHmm(time.Unix(int64(ch.LastMsgTime/1e9), 0))
	}
	...
	createdAtFormat := ""
	if ch.CreatedAt != nil {
		createdAtFormat = msutil.ToyyyyMMddHHmm(*ch.CreatedAt)
	}

	updatedAtFormat := ""
	if ch.UpdatedAt != nil {
		updatedAtFormat = msutil.ToyyyyMMddHHmm(*ch.UpdatedAt)
	}
	...
	return &channelInfoResp{
		...
		Ban:               msutil.BoolToInt(ch.Ban),
		Disband:           msutil.BoolToInt(ch.Disband),
...
func newUserResp(u msdb.User) *userResp {

	var createdAtFormat string
	var updatedAtFormat string
	if u.CreatedAt != nil {
		createdAtFormat = msutil.ToyyyyMMddHHmm(*u.CreatedAt)
	}
	if u.UpdatedAt != nil {
		updatedAtFormat = msutil.ToyyyyMMddHHmm(*u.UpdatedAt)
	}
	return &userResp{
		Id:                u.Id,
		Uid:               u.Uid,
...
func newDeviceResp(d msdb.Device) *deviceResp {

	deviceFlagFormat := ""
	switch d.DeviceFlag {
	case uint64(msproto.WEB):
		deviceFlagFormat = "WEB"
	case uint64(msproto.APP):
		deviceFlagFormat = "APP"
	case uint64(msproto.PC):
		deviceFlagFormat = "PC"
	default:
	...
	case uint8(msproto.DeviceLevelMaster):
		deviceLevelFormat = "主设备"
	case uint8(msproto.DeviceLevelSlave):
	...
	var updatedAtFormat string
	if d.CreatedAt != nil {
		createdAtFormat = msutil.ToyyyyMMddHHmm(*d.CreatedAt)
	}
	if d.UpdatedAt != nil {
		updatedAtFormat = msutil.ToyyyyMMddHHmm(*d.UpdatedAt)
	}
...
type conversationResp struct {
	Id                uint64                `json:"id"`                  // 主键
	Uid               string                `json:"uid"`                 // 用户唯一uid
	Type              msdb.ConversationType `json:"type"`                // 会话类型
...
func newConversationResp(c msdb.Conversation) *conversationResp {
	typeFormat := "聊天"
	if c.Type == msdb.ConversationTypeCMD {
		typeFormat = "命令"
	}
	var createdAtFormat string
	var updatedAtFormat string
	...
	var updatedAt int64
	if c.CreatedAt != nil {
		createdAtFormat = msutil.ToyyyyMMddHHmm(*c.CreatedAt)
		createdAt = c.CreatedAt.Unix()
	}
	if c.UpdatedAt != nil {
		updatedAtFormat = msutil.ToyyyyMMddHHmm(*c.UpdatedAt)
...
func NewLogRespFromLog(log rafttype.Log, logType LogType) (*LogResp, error) {
	...
	if logType == LogTypeConfig {
		...
		if err != nil {
			mslog.Error("config: cmd unmarshal error", zap.Error(err), zap.Uint64("index", log.Index), zap.Uint32("term", log.Term), zap.Binary("data", log.Data))
			return nil, err
		}
		cmdStr = cmd.CmdType.String()
		cmdContent, err = cmd.CMDContent()
		if err != nil {
			mslog.Error("config: cmd content error", zap.Error(err), zap.String("cmd", cmdStr), zap.Uint64("index", log.Index), zap.Uint32("term", log.Term), zap.Binary("data", cmd.Data))
			return nil, err
		}
		...
		err := cmd.Unmarshal(log.Data)
		if err != nil {
			mslog.Error("slot: cmd unmarshal error", zap.Error(err), zap.Uint64("index", log.Index), zap.Uint32("term", log.Term), zap.Binary("data", log.Data))
			return nil, err
		}
		cmdStr = cmd.CmdType.String()
		cmdContent, err = cmd.CMDContent()
		if err != nil {
			mslog.Error("slot: cmd content error", zap.Error(err), zap.String("cmd", cmdStr), zap.Uint64("index", log.Index), zap.Uint32("term", log.Term), zap.Binary("data", cmd.Data))
			return nil, err
		}
	}
	timeFormat := msutil.ToyyyyMMddHHmmss(log.Time)
...
func (r *clusterLogsReq) encode() ([]byte, error) {
	...
	enc.WriteUint8(msutil.BoolToUint8(r.OrderDesc))
...
func (r *clusterLogsReq) decode(data []byte) error {
	...
	r.OrderDesc = msutil.Uint8ToBool(OrderDesc)
