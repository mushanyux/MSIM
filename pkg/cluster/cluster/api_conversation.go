...
func (s *Server) conversationSearch(c *mshttp.Context) {
	// 搜索条件
	limit := msutil.ParseInt(c.Query("limit"))
	currentPage := msutil.ParseInt(c.Query("current_page")) // 页码
	...
	var searchLocalConversations = func() (*conversationRespTotal, error) {
		conversations, err := s.db.SearchConversation(msdb.ConversationSearchReq{
			Uid:         uid,
			Limit:       limit,
...
func (s *Server) requestConversationSearch(path string, nodeId uint64, queryMap map[string]string, headers map[string]string) (*conversationRespTotal, error) {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &conversationRespTotal)
	if err != nil {
		return nil, err
	}
