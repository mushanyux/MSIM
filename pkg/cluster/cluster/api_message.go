...
func (s *Server) messageSearch(c *mshttp.Context) {
	...
	offsetMessageId := msutil.ParseInt64(c.Query("offset_message_id"))    // 偏移的messageId
	offsetMessageSeq := msutil.ParseUint64(c.Query("offset_message_seq")) // 偏移的messageSeq（通过频道筛选并分页的时候需要传此值）
	pre := msutil.ParseInt(c.Query("pre"))                                // 是否向前搜索
	payloadStr := strings.TrimSpace(c.Query("payload"))                   // base64编码的消息内容
	messageId := msutil.ParseInt64(c.Query("message_id"))
	...
	if strings.TrimSpace(payloadStr) != "" {
		payload, err = msutil.Base64Decode(payloadStr)
		if err != nil {
	...
	var searchLocalMessage = func() ([]*messageResp, error) {
		messages, err := s.db.SearchMessages(msdb.MessageSearchReq{
			MessageId:        messageId,
			FromUid:          fromUid,
			Limit:            limit,
			ChannelId:        channelId,
...
func (s *Server) requestNodeMessageSearch(path string, nodeId uint64, queryMap map[string]string, headers map[string]string) (*messageRespTotal, error) {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &msgResp)
	if err != nil {
		return nil, err
	}
	return msgResp, nil
