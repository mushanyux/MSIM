...
func (s *Server) userSearch(c *mshttp.Context) {
	// 搜索条件
	...
	var searchLocalUsers = func() (userRespTotal, error) {
		users, err := s.db.SearchUser(msdb.UserSearchReq{
			Uid:             uid,
...
func (s *Server) requestUserSearch(path string, nodeId uint64, queryMap map[string]string, headers map[string]string) (*userRespTotal, error) {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &userRespTotal)
	if err != nil {
		return nil, err
	}
...
func (s *Server) deviceSearch(c *mshttp.Context) {
	// 搜索条件
	limit := msutil.ParseInt(c.Query("limit"))
	uid := strings.TrimSpace(c.Query("uid"))
	deviceFlag := msutil.ParseUint64(strings.TrimSpace(c.Query("device_flag")))
	nodeId := msutil.ParseUint64(c.Query("node_id"))
	pre := msutil.ParseInt(c.Query("pre"))                             // 是否向前搜索
	...
	var searchLocalDevice = func() (*deviceRespTotal, error) {
		devices, err := s.db.SearchDevice(msdb.DeviceSearchReq{
			Uid:             uid,
...
func (s *Server) requestDeviceSearch(path string, nodeId uint64, queryMap map[string]string, headers map[string]string) (*deviceRespTotal, error) {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &deviceRespTotal)
	if err != nil {
		return nil, err
	}
