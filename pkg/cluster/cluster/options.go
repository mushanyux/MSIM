...
type Options struct {
	...
	DB struct {
		MSDbShardNum     int // msdb 分片数量
		MSDbMemTableSize int // msdb MemTable大小
...
func NewOptions(opt ...Option) *Options {
	opts := &Options{
		...
		DB: struct {
			MSDbShardNum     int
			MSDbMemTableSize int
			SlotShardNum     int
			SlotMemTableSize int
		}{
			MSDbShardNum:     8,
			MSDbMemTableSize: 16 * 1024 * 1024,
			SlotShardNum:     8,
...
func WithDBMSDbShardNum(shardNum int) Option {
	return func(o *Options) {
		o.DB.MSDbShardNum = shardNum
	}
}

func WithDBMSDbMemTableSize(memTableSize int) Option {
	return func(o *Options) {
		o.DB.MSDbMemTableSize = memTableSize
	}
}
