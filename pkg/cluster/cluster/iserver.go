...
func (s *Server) GetOrCreateChannelClusterConfigFromSlotLeader(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error) {
	...
	if slotLeaderId == 0 {
		return msdb.EmptyChannelClusterConfig, fmt.Errorf("slot[%d] leader not found", s.getSlotId(channelId))
	}
...
func (s *Server) LoadOnlyChannelClusterConfig(channelId string, channelType uint8) (msdb.ChannelClusterConfig, error) {
	slotId := s.getSlotId(channelId)
	slotLeaderId := s.cfgServer.SlotLeaderId(slotId)
	if slotLeaderId == 0 {
		return msdb.EmptyChannelClusterConfig, fmt.Errorf("slot[%d] leader not found", slotId)
	}
	if s.opts.ConfigOptions.NodeId == slotLeaderId {
...
func (s *Server) Route(path string, handler msserver.Handler) {
