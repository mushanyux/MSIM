...
func (s *Server) getNodeSlotCount(nodeId uint64, cfg *types.Config) int {
	...
	for _, st := range cfg.Slots {
		if msutil.ArrayContainsUint64(st.Replicas, nodeId) {
			count++
		}
	}
	return count
...
func (s *Server) requestChannelStatus(nodeId uint64, channels []*channelBase, headers map[string]string) ([]*channelStatusResp, error) {
	...
	resp, err := network.Post(fullUrl, []byte(msutil.ToJSON(map[string]interface{}{
		"channels": channels,
	})), headers)
	if err != nil {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &statusResp)
	if err != nil {
		return nil, err
	}
...
func (s *Server) requestNodeInfo(nodeId uint64, headers map[string]string) (*NodeConfig, error) {
	...
	nodeCfg := &NodeConfig{}
	err = msutil.ReadJSONByByte([]byte(resp.Body), nodeCfg)
	if err != nil {
		return nil, err
	}
	return nodeCfg, nil
...
func getChannelTypeFormat(channelType uint8) string {
	...
	case msproto.ChannelTypeCommunity:
		return "社区"
	case msproto.ChannelTypeCommunityTopic:
		return "社区话题"
