...
import (
	...
	pb "github.com/mushanyux/MSIM/pkg/cluster/node/types"
...
func (s *Server) channelMigrate(c *mshttp.Context) {
	...
	channelType := msutil.ParseUint8(c.Param("channel_type"))
	...
	if !msutil.ArrayContainsUint64(clusterConfig.Replicas, req.MigrateFrom) {
		c.ResponseError(errors.New("MigrateFrom not in replicas"))
		return
	}

	if msutil.ArrayContainsUint64(clusterConfig.Replicas, req.MigrateTo) && req.MigrateFrom != clusterConfig.LeaderId {
	...
	if !msutil.ArrayContainsUint64(clusterConfig.Replicas, req.MigrateTo) {
		// 将要目标节点加入学习者中
...
func (s *Server) channelClusterConfig(c *mshttp.Context) {
...
func (s *Server) channelStart(c *mshttp.Context) {
...
func (s *Server) channelStop(c *mshttp.Context) {
...
func (s *Server) channelStatus(c *mshttp.Context) {
	var req struct {
...
func (s *Server) channelReplicas(c *mshttp.Context) {
	channelId := c.Param("channel_id")
	channelType := msutil.ParseUint8(c.Param("channel_type"))
	...
	for _, replicaId := range replicaIds {
		...
		if replicaId == s.opts.ConfigOptions.NodeId {
			...
			replicaResp := &channelReplicaResp{
				...
				Running:     msutil.BoolToInt(running),
				LastMsgSeq:  lastMsgSeq,
...
func (s *Server) getReplicaRoleFormatWithChannelClusterConfig(clusterConfig msdb.ChannelClusterConfig, replicaId uint64) string {
	if replicaId == clusterConfig.LeaderId {
		return "leader"
	}
	if msutil.ArrayContainsUint64(clusterConfig.Learners, replicaId) {
		return "learner"
	}
	return "follower"
}

func (s *Server) getReplicaRoleWithChannelClusterConfig(clusterConfig msdb.ChannelClusterConfig, replicaId uint64) int {
	if replicaId == clusterConfig.LeaderId {
...
func (s *Server) getReplicaRoleWithSlotClusterConfig(slot *pb.Slot, replicaId uint64) int {
	...
	if msutil.ArrayContainsUint64(slot.Learners, replicaId) {
		return int(rafttype.RoleLearner)
	}
	return int(rafttype.RoleFollower)
...
func (s *Server) channelLocalReplica(c *mshttp.Context) {
	channelId := c.Param("channel_id")
	channelType := msutil.ParseUint8(c.Param("channel_type"))
...
func (s *Server) clusterLogs(c *mshttp.Context) {
	nodeId := msutil.ParseUint64(c.Query("node_id"))
	if nodeId != s.opts.ConfigOptions.NodeId {
		c.Forward(fmt.Sprintf("%s%s", s.cfgServer.Node(nodeId).ApiServerAddr, c.Request.URL.Path))
		return
	...
	slotId := msutil.ParseUint32(c.Query("slot")) // slot id

	// 日志类型
	logType := LogType(msutil.ParseInt(c.Query("log_type")))
	if logType == LogTypeUnknown {
		logType = LogTypeConfig
	}

	pre := msutil.ParseUint64(c.Query("pre"))
	next := msutil.ParseUint64(c.Query("next"))
	limit := msutil.ParseInt(c.Query("limit"))
...
func (s *Server) clusterInfoGet(c *mshttp.Context) {
