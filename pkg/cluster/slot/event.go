...
func (s *Server) AddOrUpdateSlotRaft(slot *types.Slot) {
	...
	if !msutil.ArrayContainsUint64(slot.Replicas, s.opts.NodeId) && !msutil.ArrayContainsUint64(slot.Learners, s.opts.NodeId) {
		if rft != nil {
...
func (s *Server) slotToConfig(st *types.Slot) rafttype.Config {
	...
	} else if msutil.ArrayContainsUint64(st.Learners, s.opts.NodeId) {
