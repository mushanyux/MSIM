...
import (
	...
	"github.com/mushanyux/MSIM/pkg/cluster/slot/key"
...
func NewPebbleShardLogStorage(s *Server, path string, shardNum uint32) *PebbleShardLogStorage {
	return &PebbleShardLogStorage{
		...
		Log:     mslog.NewMSLog(fmt.Sprintf("PebbleShardLogStorage[%s]", path)),
...
func (p *PebbleShardLogStorage) shardBatchDB(v string) *msdb.BatchDB {
	shardId := p.shardId(v)
...
func (p *PebbleShardLogStorage) shardBatchDBWithIndex(index uint32) *msdb.BatchDB {
...
// 	dbDir := path.Join(opts.DataDir, "localMushanimdb")
// 	return &localStorage{
// 		opts:  opts,
// 		dbDir: dbDir,
// 		Log:   mslog.NewMSLog(fmt.Sprintf("localStorage[%s]", dbDir)),
// 		wo: &pebble.WriteOptions{
// 			Sync: true,
// 		},
// 	}
...
// 	return msutil.GetSlotFillFormat(int(slotID), int(l.opts.SlotCount))
// }

// func (l *localStorage) getSlotFillFormatMax() string {
// 	return msutil.GetSlotFillFormat(int(l.opts.SlotCount), int(l.opts.SlotCount))
// }

// func (l *localStorage) getSlotFillFormatMin() string {
// 	return msutil.GetSlotFillFormat(0, int(l.opts.SlotCount))
// }

// func (l *localStorage) getChannelSlotId(channelId string) uint32 {
// 	return msutil.GetSlotNum(int(l.opts.SlotCount), channelId)
// }
