...
func ToFrame(packet interface{}) (msproto.Frame, string, error) {
	...
	case SendRequest:
		return p.Params.ToProto(), p.ID, nil
	case PingRequest:
		return &msproto.PingPacket{}, p.ID, nil
...
func FromFrame(reqId string, frame msproto.Frame) (interface{}, error) {

	switch frame.GetFrameType() {
	case msproto.CONNACK:
	...
	case msproto.SENDACK:
		sendack := frame.(*msproto.SendackPacket)
	...
	case msproto.RECV:
		recv := frame.(*msproto.RecvPacket)
		result := FromProtoRecvNotification(recv)
		return result, nil
	case msproto.DISCONNECT:
		disconnect := frame.(*msproto.DisconnectPacket)
