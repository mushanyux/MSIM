...
func (o *Options) requestPrometheus(query string, r v1.Range, opt ...v1.Option) (model.Value, error) {
	...
	if o.prometheusClient == nil {
		...
		if err != nil {
			mslog.Error("create prometheus client failed", zap.Error(err))
			return nil, err
		}
	...
	if err != nil {
		mslog.Error("query prometheus failed", zap.Error(err))
		return nil, err
	}
	if len(warnings) > 0 {
		mslog.Warn("query prometheus warnings", zap.Any("warnings", warnings))
	}
	return result, nil
}
