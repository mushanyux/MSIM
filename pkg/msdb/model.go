...
func (m *Message) Marshal() ([]byte, error) {
	data, err := proto.EncodeFrame(&m.RecvPacket, msproto.LatestVersion)
	if err != nil {
		return nil, err
	}

	// TODO: 这里msproto.LatestVersion + fixVersion是为了兼容老版本编码，如果第一个版本号大于100，那么就是新版本编码
	// 所以这里msproto.LatestVersion不能大于156，否则uint8会溢出，实际中msproto.LatestVersion应该不会太大
	fixVersion := uint8(100)

	enc := msproto.NewEncoder()
	defer enc.End()
	enc.WriteUint8(msproto.LatestVersion + fixVersion)
...
func (c *ChannelInfo) Unmarshal(data []byte) error {
	...
	c.Ban = msutil.Uint8ToBool(ban)
	c.Large = msutil.Uint8ToBool(large)
	c.Disband = msutil.Uint8ToBool(disband)
