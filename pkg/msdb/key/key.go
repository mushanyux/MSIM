...
func NewMessageSearchLomseWith(channelId string, channelType uint8, messageSeq uint64) []byte {
	key := make([]byte, 20)
	key[0] = TableMessage.Id[0]
	key[1] = TableMessage.Id[1]
	key[2] = dataTypeTable
...
// NewAllowlistIndexUidLomsey 创建一个uid唯一索引的low key
func NewAllowlistIndexUidLomsey(channelId string, channelType uint8) []byte {
	key := make([]byte, TableAllowlist.IndexSize)
	key[0] = TableAllowlist.Id[0]
	key[1] = TableAllowlist.Id[1]
	key[2] = dataTypeIndex
...
func NewConversationLocalUserLomsey(channelId string, channelType uint8) []byte {
	key := make([]byte, 13)
	key[0] = TableConversationLocalUser.Id[0]
	key[1] = TableConversationLocalUser.Id[1]
	key[2] = dataTypeTable
