...
// OnRecv 收到消息事件
type OnRecv func(recv *msproto.RecvPacket) error
type OnSendack func(sendackPacket *msproto.SendackPacket)
...
func New(addr string, opt ...Option) *Client {
	...
	c := &Client{
		...
		opts:  opts,
		proto: msproto.New(),
		Log:   mslog.NewMSLog(fmt.Sprintf("IMClient[%s]", opts.UID)),
		writer: &limWriter{
...
func (c *Client) sendPing(ch chan struct{}) error {
	...
	data, err := c.proto.EncodeFrame(&msproto.PingPacket{}, c.opts.ProtoVersion)
	if err != nil {
		return err
	}
...
func (c *Client) handlePacket(frame msproto.Frame) error {

	c.InBytes.Add(uint64(frame.GetFrameSize()))
	c.InMsgs.Inc()

	switch frame.GetFrameType() {
	case msproto.SENDACK: // 发送回执
		c.handleSendackPacket(frame.(*msproto.SendackPacket))
	case msproto.RECV: // 收到消息
		c.handleRecvPacket(frame.(*msproto.RecvPacket))
	case msproto.PONG: // pong
		c.handlePong()
	}
	return nil
}

func (c *Client) handleSendackPacket(packet *msproto.SendackPacket) {
	if c.onSendack != nil {
		c.onSendack(packet)
	}
}
...
// 处理接受包
func (c *Client) handleRecvPacket(packet *msproto.RecvPacket) {
	var err error
	var payload []byte
	if c.onRecv != nil {
		if !packet.Setting.IsSet(msproto.SettingNoEncrypt) {
			payload, err = msutil.AesDecryptPkcs7Base64(packet.Payload, []byte(c.aesKey), []byte(c.salt))
			if err != nil {
	...
	if err == nil {
		c.sendPacket(&msproto.RecvackPacket{
...
func (c *Client) SendMessage(channel *Channel, payload []byte, opt ...SendOption) error {
	...
	if !opts.NoEncrypt {
		// 加密消息内容
		newPayload, err = msutil.AesEncryptPkcs7Base64(payload, []byte(c.aesKey), []byte(c.salt))
		if err != nil {
			c.Error("加密消息payload失败！", zap.Error(err), zap.String("aesKey", c.aesKey), zap.String("salt", c.salt))
			return err
		}
	} else {
		setting.Set(msproto.SettingNoEncrypt)
	}

	clientMsgNo := opts.ClientMsgNo
	if clientMsgNo == "" {
		clientMsgNo = msutil.GenUUID() // TODO: uuid生成非常耗性能
	}
	clientSeq := c.clientIDGen.Add(1)
	packet := &msproto.SendPacket{
	...
	if !opts.NoEncrypt {
		...
		actMsgKey, err := msutil.AesEncryptPkcs7Base64([]byte(signStr), []byte(c.aesKey), []byte(c.salt))
		if err != nil {
			c.Error("加密数据失败！", zap.Error(err))
			return err
		}
		packet.MsgKey = msutil.MD5(string(actMsgKey))
	}
...
func (c *Client) sendConnect() error {
	...
	c.clientPrivKey, clientPubKey = msutil.GetCurve25519KeyPair() // 生成服务器的DH密钥对
	packet := &msproto.ConnectPacket{
		Version:         c.opts.ProtoVersion,
		DeviceID:        msutil.GenUUID(),
		DeviceFlag:      msproto.APP,
	...
	connack, ok := f.(*msproto.ConnackPacket)
	if !ok {
	...
	shareKey := msutil.GetCurve25519Key(c.clientPrivKey, serverPubKey) // 共享key
	c.aesKey = msutil.MD5(base64.StdEncoding.EncodeToString(shareKey[:]))[:16]
	c.status = CONNECTED
...
// 发送包
func (c *Client) sendPacket(packet msproto.Frame) error {
...
func (c *Client) appendPacket(packet msproto.Frame) error {
