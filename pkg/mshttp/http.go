...
func NewWithLogger(loggerHandler HandlerFunc) *MSHttp {
	l := &MSHttp{
		r:    gin.New(),
		pool: sync.Pool{},
	}
...
func (l *MSHttp) GetGinRoute() *gin.Engine {
	return l.r
}
...
func (l *MSHttp) LMHttpHandler(handlerFunc HandlerFunc) gin.HandlerFunc {
...
// DELETE DELETE
func (l *MSHttp) DELETE(relativePath string, handlers ...HandlerFunc) {
...
// Group Group
func (l *MSHttp) Group(relativePath string, handlers ...HandlerFunc) {
	l.r.Group(relativePath, l.handlersToGinHandleFunc(handlers)...)
}

func (l *MSHttp) handlersToGinHandleFunc(handlers []HandlerFunc) []gin.HandlerFunc {
	newHandlers := make([]gin.HandlerFunc, 0, len(handlers))
	for _, handler := range handlers {
		newHandlers = append(newHandlers, l.LMHttpHandler(handler))
	}
