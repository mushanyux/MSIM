...
func (s *Server) onData(conn msnet.Conn) error {
...
func (s *Server) handleAuthenticatedConn(conn msnet.Conn, connCtx *eventbus.Conn, buff []byte, isJson bool) error {
	offset := 0
	var events []*eventbus.Event

	frames := make([]msproto.Frame, 0, 10)
	...
	for i, frame := range frames {
		...
		if frame.GetFrameType() == msproto.SEND {
...
func (s *Server) handleUnauthenticatedConn(conn msnet.Conn, buff []byte, isJson bool) (ctx *eventbus.Conn, consumedBytes int, err error) {
	dataOffset := 0
	...
	var connectPacket *msproto.ConnectPacket
	...
	if isJson && !options.G.DisableJSONRPC {
		...
		if packet.GetFrameType() != msproto.CONNECT {
		...
		var ok bool
		connectPacket, ok = packet.(*msproto.ConnectPacket)
		if !ok {
	...
	connCtx := &eventbus.Conn{
		...
		DeviceFlag:   msproto.DeviceFlag(connectPacket.DeviceFlag),
...
func unpacket(buff []byte) ([]byte, msproto.FrameType) {
	if len(buff) <= 0 {
		return nil, 0
	}
	offset := 0 // 偏移位置
	var packetType msproto.FrameType
	for len(buff) > offset {
		typeAndFlags := buff[offset]
		packetType = msproto.FrameType(typeAndFlags >> 4)
		if packetType == msproto.PING || packetType == msproto.PONG {
			offset++
			continue
		}
