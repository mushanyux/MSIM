...
func NewService() *Service {
	return &Service{
		...
		Log:         mslog.NewMSLog("common.Service"),
...
// GetOrRequestAndMakeTag 获取或请求tag
...
func (s *Service) GetOrRequestAndMakeTag(fakeChannelId string, channelType uint8, tagKey string, targetNodeId uint64) (*types.Tag, error) {
	...
	if err != nil {
		mslog.Error("GetOrRequestTag: getLeaderOfChannel failed", zap.Error(err), zap.String("channelId", fakeChannelId), zap.Uint8("channelType", channelType))
		return nil, err
	}
	if leader == nil {
		mslog.Warn("GetOrRequestTag: leader is nil", zap.String("channelId", fakeChannelId), zap.Uint8("channelType", channelType))
	...
	if options.G.IsLocalNode(leader.Id) {
		mslog.Warn("tag not exist in leader node", zap.String("tagKey", tagKey), zap.String("fakeChannelId", fakeChannelId), zap.Uint8("channelType", channelType))
...
func (s *Service) getOrMakePersonTag(fakeChannelId string) (*types.Tag, error) {
	...
	service.TagManager.SetChannelTag(fakeChannelId, msproto.ChannelTypePerson, tag.Key)
	return tag, nil
}

// 检查连接的真实性 并获取真实连接
func CheckConnValidAndGetRealConn(conn *eventbus.Conn) (msnet.Conn, error) {
	// 获取到真实连接
