...
func NewMigrateTask(s *Server) *MigrateTask {
	return &MigrateTask{
		...
		Log:            mslog.NewMSLog("MigrateTask"),
		goroutineCount: 20,
	}
}
...
func (m *MigrateTask) IsMigrated() bool {
	return msutil.FileExists(path.Join(options.G.DataDir, "migrated"))
}

func (m *MigrateTask) Migrated() {
	_ = msutil.WriteFile(path.Join(options.G.DataDir, "migrated"), []byte("1"))
}
...
func (m *MigrateTask) stepUserImport() {
	...
	uids = msutil.RemoveRepeatedElement(uids)
...
func (m *MigrateTask) getChannelFromOldVersion() ([]*mgChannelResp, error) {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &channels)
	if err != nil {
		return nil, err
	}
	return channels, nil
...
func (m *MigrateTask) getUserFromOldVersion() ([]*mgUserResp, error) {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &users)
	if err != nil {
		return nil, err
	}
	return users, nil
...
func (m *MigrateTask) getTopicsFromOldVersion(slotId uint32) ([]string, error) {
	...
	resp, err := network.Post(m.getFullUrl("/migrate/topics"), []byte(msutil.ToJSON(map[string]interface{}{
		"slot": slotId,
	})), nil)
	if err != nil {
		return nil, err
	...
	var topics []string
	err = msutil.ReadJSONByByte([]byte(resp.Body), &topics)
	if err != nil {
		return nil, err
	}
	return topics, nil
...
func (m *MigrateTask) importUser(uid string) error {
	...
	relationData, err := m.getChannelRelationData(uid, msproto.ChannelTypePerson)
	if err != nil {
		return err
	}
	...
	if len(relationData.Allowlist) > 0 {
		members := make([]msdb.Member, 0, len(relationData.Allowlist))
		createdAt := time.Now()
		updatedAt := time.Now()
		...
		err = service.Store.AddAllowlist(uid, msproto.ChannelTypePerson, members)
		if err != nil {
			return err
		}
	}
	...
	if len(relationData.Denylist) > 0 {
		...
		members := make([]msdb.Member, 0, len(relationData.Denylist))
		createdAt := time.Now()
		updatedAt := time.Now()
		...
		err = service.Store.AddDenylist(uid, msproto.ChannelTypePerson, members)
		if err != nil {
			return err
		}
	}
	...
	dbConversations := make([]msdb.Conversation, 0, len(conversations))
	for _, conversation := range conversations {
		...
		dbConversations = append(dbConversations, msdb.Conversation{
			Uid:          uid,
...
func (m *MigrateTask) importChannel(channel *mgChannelResp) error {
	...
	createdAt := time.Now()
	updatedAt := time.Now()
	err = service.Store.AddChannelInfo(msdb.ChannelInfo{
	...
	if len(relationData.Subscribers) > 0 {
		members := make([]msdb.Member, 0, len(relationData.Subscribers))
		createdAt := time.Now()
		updatedAt := time.Now()
...
func (m *MigrateTask) importMessage(topic string) error {
	...
	channelType := msutil.ParseUint8(topicSplits[0])
	...
	for {
		...
		messages := make([]msdb.Message, 0, len(resp.Messages))
		for _, msg := range resp.Messages {
			if msg.FromUID == "" {
				msg.FromUID = options.G.SystemUID
			}
			if msg.ChannelType == msproto.ChannelTypePerson {
...
func newDBMessage(m *types.MessageResp) msdb.Message {
	return msdb.Message{
		RecvPacket: msproto.RecvPacket{
			...
			Setting:     msproto.Setting(m.Setting),
...
func (m *MigrateTask) syncMessages(channelId string, channelType uint8, startMessageSeq, endMessageSeq uint32, limit int) (*syncMessageResp, error) {
	...
	resp, err := network.Post(m.getFullUrl("/channel/messagesync"), []byte(msutil.ToJSON(map[string]interface{}{
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &syncResp)
	if err != nil {
		return nil, err
	}
...
func (m *MigrateTask) getChannelRelationData(channelId string, channelType uint8) (*mgChannelRelationResp, error) {
	...
	resp, err := network.Post(m.getFullUrl("/migrate/channel"), []byte(msutil.ToJSON(map[string]interface{}{
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &relation)
	if err != nil {
		return nil, err
	}
	return relation, nil
...
func (m *MigrateTask) getConversations(uid string) ([]*syncUserConversationResp, error) {
	resp, err := network.Post(m.getFullUrl("/conversation/sync"), []byte(msutil.ToJSON(map[string]interface{}{
		"uid":       uid,
		"msg_count": 1,
	})), nil)
	if err != nil {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &conversations)
	if err != nil {
		return nil, err
	}
