...
import (
	...
	"github.com/mushanyux/MSIM/pkg/pse"
...
func (v *varz) route(r *mshttp.MSHttp) {
	r.GET("/varz", v.HandleVarz) // 获取系统变量

	r.GET("/varz/setting", v.Settings) // 获取系统设置
}

func (v *varz) HandleVarz(c *mshttp.Context) {
...
func (v *varz) Settings(c *mshttp.Context) {
	...
	setting.PrometheusOn = msutil.BoolToInt(options.G.PrometheusOn())
	setting.StressOn = msutil.BoolToInt(options.G.Stress)
...
func CreateVarz(s *Server) *Varz {
	...
	return &Varz{
		...
		ManagerOn:   msutil.BoolToInt(opts.Manager.On),
		...
		ManagerTokenOn: msutil.BoolToInt(opts.ManagerTokenOn),
	}
}
