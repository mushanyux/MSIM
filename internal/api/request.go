...
func (s *request) requestSyncMessage(nodeID uint64, reqs []*channelRecentMessageReq, uid string, msgCount int, orderByLast bool) ([]*channelRecentMessage, error) {
	...
	request := rest.Request{
		...
		Body: []byte(msutil.ToJSON(map[string]interface{}{
			...
			"msg_count":     msgCount,
			"channels":      reqs,
			"order_by_last": msutil.BoolToInt(orderByLast),
		})),
	}
	...
	if err := msutil.ReadJSONByByte([]byte(resp.Body), &results); err != nil {
		return nil, err
	}
	return results, nil
}
