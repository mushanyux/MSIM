...
// Route Route
func (ch *channel) route(r *mshttp.MSHttp) {
	//################### 频道 ###################
...
func (ch *channel) channelCreateOrUpdate(c *mshttp.Context) {
	...
	// channelInfo := msstore.NewChannelInfo(req.ChannelID, req.ChannelType)
...
// 更新或添加频道信息
func (ch *channel) updateOrAddChannelInfo(c *mshttp.Context) {
...
func (ch *channel) addSubscriber(c *mshttp.Context) {
	...
	if req.ChannelType == 0 {
		req.ChannelType = msproto.ChannelTypeGroup //默认为群
	}
	leaderInfo, err := service.Cluster.SlotLeaderOfChannel(req.ChannelId, req.ChannelType) // 获取频道的领导节点
	if err != nil {
		ch.Error("获取频道所在节点失败！", zap.Error(err), zap.String("channelID", req.ChannelId), zap.Uint8("channelType", req.ChannelType))
	...
	if !exist { // 如果没有频道则创建
		...
		channelInfo := msdb.NewChannelInfo(req.ChannelId, req.ChannelType)
...
func (ch *channel) addSubscriberWithReq(req subscriberAddReq) error {
	...
	if len(newSubscribers) > 0 {
		...
		members := make([]msdb.Member, 0, len(newSubscribers))
		createdAt := time.Now()
		updatedAt := time.Now()
		for _, subscriber := range newSubscribers {
			members = append(members, msdb.Member{
				Uid:       subscriber,
				CreatedAt: &createdAt,
				UpdatedAt: &updatedAt,
			})
		...
		if req.ChannelType != msproto.ChannelTypeLive { // 直播频道不添加会话
			conversations := make([]msdb.Conversation, 0, len(newSubscribers))
			for _, subscriber := range newSubscribers {
				createdAt := time.Now()
				updatedAt := time.Now()
...
func (ch *channel) addSubscribers(channelId string, channelType uint8, members []msdb.Member) error {
...
func (ch *channel) removeSubscriber(c *mshttp.Context) {
	...
	// 删除订阅者的会话缓存
	if req.ChannelType != msproto.ChannelTypeLive { // 直播频道不处理最近会话
...
func (ch *channel) setTmpSubscriber(c *mshttp.Context) {
	...
	leaderInfo, err := service.Cluster.LeaderOfChannel(req.ChannelId, msproto.ChannelTypeTemp) // 获取频道的领导节点
	if err != nil {
		ch.Error("获取频道所在节点失败！", zap.Error(err), zap.String("channelID", req.ChannelId), zap.Uint8("channelType", msproto.ChannelTypeTemp))
		c.ResponseError(errors.New("获取频道所在节点失败！"))
		return
	}
	leaderIsSelf := leaderInfo.Id == options.G.Cluster.NodeId
...
func setTmpSubscriberWithReq(req tmpSubscriberSetReq) error {
	...
	service.TagManager.SetChannelTag(req.ChannelId, msproto.ChannelTypeTemp, tag.Key)
	return nil
}

func (ch *channel) blacklistAdd(c *mshttp.Context) {
	...
	members := make([]msdb.Member, 0, len(req.UIDs))
	createdAt := time.Now()
	updatedAt := time.Now()
...
func (ch *channel) blacklistSet(c *mshttp.Context) {
	var req blacklistReq
	bodyBytes, err := BindJSON(&req, c)
	if err != nil {
		ch.Error("数据格式有误！", zap.Error(err))
...
func (ch *channel) blacklistRemove(c *mshttp.Context) {
	var req blacklistReq
	bodyBytes, err := BindJSON(&req, c)
	if err != nil {
		ch.Error("数据格式有误！", zap.Error(err))
	...
	if req.ChannelType != msproto.ChannelTypeLive { // 直播频道不处理最近会话
		...
		updatedAt := time.Now()
		for _, uid := range req.UIDs {
			err = service.Store.AddConversationsIfNotExist([]msdb.Conversation{
				{
					Uid:         uid,
					Type:        msdb.ConversationTypeChat,
					ChannelId:   req.ChannelId,
...
// 删除频道
func (ch *channel) channelDelete(c *mshttp.Context) {
	...
	if msdb.IsEmptyChannelInfo(channelInfo) {
...
// 添加白名单
func (ch *channel) whitelistAdd(c *mshttp.Context) {
...
func (ch *channel) whitelistSet(c *mshttp.Context) {
	var req whitelistReq
	bodyBytes, err := BindJSON(&req, c)
	if err != nil {
		ch.Error("数据格式有误！", zap.Error(err))
...
// 移除白名单
func (ch *channel) whitelistRemove(c *mshttp.Context) {
	var req whitelistReq
	bodyBytes, err := BindJSON(&req, c)
	if err != nil {
		ch.Error("数据格式有误！", zap.Error(err))
...
func (ch *channel) whitelistGet(c *mshttp.Context) {
	channelId := c.Query("channel_id")
	channelType := msutil.ParseUint8(c.Query("channel_type"))
...
// 同步频道内的消息
func (ch *channel) syncMessages(c *mshttp.Context) {
...
func (ch *channel) getChannelMaxMessageSeq(c *mshttp.Context) {
	channelId := c.Query("channel_id")
	channelType := msutil.StringToUint8(c.Query("channel_type"))
...
func (ch *channel) addOrUpdateChannel(channelInfo msdb.ChannelInfo) error {
	existChannel, err := service.Store.GetChannel(channelInfo.ChannelId, channelInfo.ChannelType)
	if err != nil && err != msdb.ErrNotFound {
		return err
	}

	if msdb.IsEmptyChannelInfo(existChannel) {
		err = service.Store.AddChannelInfo(channelInfo)
		if err != nil {
			return err
		}
