...
func newConnz(s *Server) *connz {
	return &connz{
		Log: mslog.NewMSLog("ConnzAPI"),
		s:   s,
	}
}

func (co *connz) route(r *mshttp.MSHttp) {
	r.GET("/connz", co.HandleConnz)
}

func (co *connz) HandleConnz(c *mshttp.Context) {
...
func device(connCtx *connzResp) string {
	d := "未知"
	level := "主"
	switch msproto.DeviceFlag(connCtx.DeviceFlag) {
	case msproto.APP:
		d = "App"
	case msproto.PC:
		d = "PC"
	...
	if msproto.DeviceLevel(connCtx.DeviceLevel) == msproto.DeviceLevelSlave {
		level = "从"
	}
