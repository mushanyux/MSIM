...
func newUser(s *Server) *user {
	return &user{
		Log: mslog.NewMSLog("user"),
		s:   s,
	}
}

// Route 用户相关路由配置
func (u *user) route(r *mshttp.MSHttp) {
...
// 强制设备退出
func (u *user) deviceQuit(c *mshttp.Context) {
	var req struct {
	...
	if err != nil {
		u.Error("获取频道所在节点失败！", zap.Error(err), zap.String("channelID", req.UID), zap.Uint8("channelType", msproto.ChannelTypePerson))
	...
	if req.DeviceFlag == -1 {
		_ = u.quitUserDevice(req.UID, msproto.APP)
		_ = u.quitUserDevice(req.UID, msproto.WEB)
		_ = u.quitUserDevice(req.UID, msproto.PC)
	} else {
		_ = u.quitUserDevice(req.UID, msproto.DeviceFlag(req.DeviceFlag))
	}
...
func (u *user) quitUserDevice(uid string, deviceFlag msproto.DeviceFlag) error {
	...
	if len(oldConns) > 0 {
		for _, oldConn := range oldConns {
			eventbus.User.ConnWrite("", oldConn, &msproto.DisconnectPacket{
...
func (u *user) getOnlineStatus(c *mshttp.Context) {
	var uids []string
	err := c.BindJSON(&uids)
	if err != nil {
		u.Error("数据格式有误！", zap.Error(err))
...
func (u *user) getOnlineConnsForCluster(uids []string) ([]*OnlinestatusResp, error) {
	...
	for _, uid := range uids {
		leaderInfo, err := service.Cluster.SlotLeaderOfChannel(uid, msproto.ChannelTypePerson) // 获取频道的领导节点
		if err != nil {
			u.Error("获取频道所在节点失败！", zap.Error(err), zap.String("channelID", uid), zap.Uint8("channelType", msproto.ChannelTypePerson))
...
func (u *user) requestOnlineStatus(nodeId uint64, uids []string) ([]*OnlinestatusResp, error) {
	...
	resp, err := network.Post(reqURL, []byte(msutil.ToJSON(uids)), nil)
	if err != nil {
	...
	err = msutil.ReadJSONByByte([]byte(resp.Body), &onlineStatusResps)
	if err != nil {
...
// 更新用户的token
func (u *user) updateToken(c *mshttp.Context) {
	...
	channelInfo, err := service.Store.GetChannel(req.UID, msproto.ChannelTypePerson)
	if err != nil {
	...
	if ban {
		c.ResponseStatus(int(msproto.ReasonBan))
		return
	}
	...
	// 如果用户不存在，则添加用户
	if msdb.IsEmptyUser(user) {
		createdAt := time.Now()
		updatedAt := time.Now()
		err = service.Store.AddUser(msdb.User{
			Uid:       req.UID,
			CreatedAt: &createdAt,
			UpdatedAt: &updatedAt,
		})
	...
	if msdb.IsEmptyDevice(device) {
		createdAt := time.Now()
		err = service.Store.AddDevice(msdb.Device{
	...
	if req.DeviceLevel == msproto.DeviceLevelMaster {
		// 如果存在旧连接，则发起踢出请求
...
// 添加系统uid
func (u *user) systemUidsAdd(c *mshttp.Context) {
	var req struct {
...
func (u *user) requestSystemUidsAddToCache(nodeInfo *types.Node, uids []string) error {
	...
	resp, err := network.Post(reqURL, []byte(msutil.ToJSON(map[string]interface{}{
		"uids": uids,
	})), nil)
	if err != nil {
...
func (u *user) systemUidsAddToCache(c *mshttp.Context) {
	var req struct {
		UIDs []string `json:"uids"`
	}
...
// 移除系统uid
func (u *user) systemUidsRemove(c *mshttp.Context) {
	var req struct {
		UIDs []string `json:"uids"`
	}
	bodyBytes, err := BindJSON(&req, c)
...
func (u *user) systemUidsRemoveFromCache(c *mshttp.Context) {
	var req struct {
		UIDs []string `json:"uids"`
	}
	if err := c.BindJSON(&req); err != nil {
...
func (u *user) getSystemUids(c *mshttp.Context) {
...
func (u *user) updatePluginNo(c *mshttp.Context) {
	var req struct {
		UID      string `json:"uid"`
	...
	leaderInfo, err := service.Cluster.SlotLeaderOfChannel(req.UID, msproto.ChannelTypePerson) // 获取频道的领导节点
	if err != nil {
		u.Error("updatePluginNo: 获取频道所在节点失败！", zap.Error(err), zap.String("channelID", req.UID), zap.Uint8("channelType", msproto.ChannelTypePerson))
		c.ResponseError(errors.New("获取频道所在节点失败！"))
		return
	}
	leaderIsSelf := leaderInfo.Id == options.G.Cluster.NodeId
...
type UpdateTokenReq struct {
	...
	DeviceFlag  msproto.DeviceFlag  `json:"device_flag"`  // 设备标识  0.app 1.web
	DeviceLevel msproto.DeviceLevel `json:"device_level"` // 设备等级 0.为从设备 1.为主设备
}
