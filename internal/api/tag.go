...
func newTag(s *Server) *tag {
	return &tag{
		s:   s,
		Log: mslog.NewMSLog("tag"),
	}
}

func (t *tag) route(r *mshttp.MSHttp) {
...
func (t *tag) get(c *mshttp.Context) {
...
func (t *tag) remove(c *mshttp.Context) {
	var req struct {
...
func (t *tag) list(c *mshttp.Context) {
...
func newTagResp(tag *types.Tag) *tagResp {
	...
	if !tag.CreatedAt.IsZero() {
		createdAtFormat = msutil.ToyyyyMMddHHmm(tag.CreatedAt)
	}

	var expireAt string
	if !tag.LastGetTime.IsZero() {
		expireAt = msutil.ToyyyyMMddHHmm(tag.LastGetTime.Add(options.G.Tag.Expire))
	}
	var lastGetAt string
	if !tag.LastGetTime.IsZero() {
		lastGetAt = msutil.ToyyyyMMddHHmm(tag.LastGetTime)
	}
