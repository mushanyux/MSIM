...
func newStress(s *Server) *stress {
	return &stress{
		s:   s,
		Log: mslog.NewMSLog("stress"),
	}
}

// Route route
func (s *stress) route(r *mshttp.MSHttp) {
...
func (s *stress) add(c *mshttp.Context) {
	var req struct {
	...
	// 保存压测机数据
	nw := time.Now()
	err = service.Store.AddOrUpdateTester(msdb.Tester{
		No:        no,
		Addr:      req.Addr,
		CreatedAt: &nw,
		UpdatedAt: &nw,
...
func (s *stress) remove(c *mshttp.Context) {
	var req struct {
...
func (s *stress) infoList(c *mshttp.Context) {
...
func (s *stress) start(c *mshttp.Context) {
	var req struct {
		taskCfg
...
func (s *stress) stop(c *mshttp.Context) {
	var req struct {
		No string `json:"no"`
	}
	if err := c.BindJSON(&req); err != nil {
...
func (s *stress) report(c *mshttp.Context) {
	var req struct {
		No string `json:"no"`
	}
	if err := c.BindJSON(&req); err != nil {
...
// 获取模块
func (s *stress) templates(c *mshttp.Context) {
	templates := []template{
		{
			Name: "性能测试-发送速率",
			Task: &taskCfg{
...
func (s *stress) requestExchange(addr string, server string) (string, error) {
	...
	resp, err := network.Post(strings.TrimSuffix(addr, "/")+"/v1/exchange", []byte(msutil.ToJSON(map[string]interface{}{
		"server": server,
	})), nil)
	if err != nil {
		return "", err
	...
	if err := msutil.ReadJSONByByte([]byte(resp.Body), &resultMap); err != nil {
		return "", err
	}
...
func (s *stress) requestSystemInfo(addr string) (*testerInfoResp, error) {
	...
	if err := msutil.ReadJSONByByte([]byte(resp.Body), &testerResp); err != nil {
		return nil, err
	}
...
func (s *stress) requestStart(addr string, task *taskCfg) error {
	resp, err := network.Post(strings.TrimSuffix(addr, "/")+"/v1/stress/start", []byte(msutil.ToJSON(task)), nil)
	if err != nil {
		return err
	}
	if resp.StatusCode != http.StatusOK {
...
func (s *stress) requestReport(addr string) (*statsResp, error) {
	...
	var stats *statsResp
	if err := msutil.ReadJSONByByte([]byte(resp.Body), &stats); err != nil {
		return nil, err
	}
...
func newTesterInfoResp(t msdb.Tester) *testerInfoResp {
