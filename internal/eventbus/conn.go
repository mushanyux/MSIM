...
type Conn struct {
	...
	DeviceId string
	// 对应设备的标记
	DeviceFlag msproto.DeviceFlag
	// 对应设备的等级
	DeviceLevel msproto.DeviceLevel
	// 连接是否通过认证
	Auth bool
	// 连接的aes iv
	AesIV []byte
...
func (c *Conn) Decode(data []byte) error {
	...
	c.DeviceFlag = msproto.DeviceFlag(deviceFlag)

	var deviceLevel uint8
	if deviceLevel, err = dec.Uint8(); err != nil {
		return err
	}
	c.DeviceLevel = msproto.DeviceLevel(deviceLevel)

	var auth uint8
	if auth, err = dec.Uint8(); err != nil {
		return err
	}
	c.Auth = msutil.Uint8ToBool(auth)
	...
	c.IsJsonRpc = msutil.Uint8ToBool(isJsonRpc)
