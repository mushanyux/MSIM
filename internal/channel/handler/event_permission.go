...
func (h *Handler) permission(ctx *eventbus.ChannelContext) {
	...
	for _, event := range events {
		...
		if err != nil {
			...
			reasonCode = msproto.ReasonSystemError
		} else if reasonCode != msproto.ReasonSuccess {
...
func (h *Handler) hasPermissionForChannel(channelId string, channelType uint8) (msproto.ReasonCode, error) {
	...
	if channelInfo.SendBan {
		return msproto.ReasonSendBan, nil
	}
	// 频道已解散
	if channelInfo.Disband {
		return msproto.ReasonDisband, nil
	}
	return msproto.ReasonSuccess, nil
}
...
// 通用频道权限判断
func (h *Handler) hasPermissionForCommChannel(channelId string, channelType uint8, e *eventbus.Event) (msproto.ReasonCode, error) {
	var (
	...
	if !isSubscriber {
		return msproto.ReasonSubscriberNotExist, nil
	}
...
// 个人频道权限判断
func (h *Handler) hasPermissionForPerson(channelId string, _ uint8, e *eventbus.Event) (msproto.ReasonCode, error) {
	var (
...
func (h *Handler) requestAllowSend(from, to string) (msproto.ReasonCode, error) {
	...
	if resp.Status == proto.StatusError {
		return msproto.ReasonSystemError, errors.New(string(resp.Body))
	}
	return msproto.ReasonCode(resp.Status), nil
}
...
func (h *Handler) allowSend(from, to string) (msproto.ReasonCode, error) {
	...
	// 判断是否是黑名单内
	isDenylist, err := service.Store.ExistDenylist(to, msproto.ChannelTypePerson, from)
	if err != nil {
	...
	if !toChannelInfo.AllowStranger && !options.G.WhitelistOffOfPerson {
		isAllowlist, err := service.Store.ExistAllowlist(to, msproto.ChannelTypePerson, from)
		if err != nil {
			h.Error("ExistAllowlist error", zap.Error(err))
			return msproto.ReasonSystemError, err
		}
