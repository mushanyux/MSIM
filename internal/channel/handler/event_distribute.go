...
// 跳过会话更新的频道类型
var skipConversationUpdateChannelTypes = []uint8{msproto.ChannelTypeData, msproto.ChannelTypeTemp, msproto.ChannelTypeLive}
...
func (h *Handler) distributeByTag(leaderId uint64, tag *types.Tag, channelId string, channelType uint8, events []*eventbus.Event) {
	...
	if len(offlineUids) > 0 {
		...
		for _, event := range events {
			...
			// 移除重复的离线用户
			filteredOfflineUids = msutil.RemoveRepeatedElement(filteredOfflineUids)
...
func (h *Handler) makeChannelTag(fakeChannelId string, channelType uint8) (*types.Tag, error) {
	...
	if channelType == msproto.ChannelTypePerson { // 个人频道
...
func (h *Handler) deviceOnlineStatus(uid string) (bool, bool) {
	...
	for _, conn := range toConns {
		if conn.DeviceLevel == msproto.DeviceLevelMaster {
			masterIsOnline = true
			break
		}
	}
