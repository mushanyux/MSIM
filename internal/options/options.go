...
type Options struct {
	...
	Proto msproto.Protocol // 悟空IM protocol
...
func (o *Options) ConfigureWithViper(vp *viper.Viper) {
	...
	if strings.TrimSpace(externalIp) == "" && o.External.AutoGetExternalIP { // 开启了自动获取外网ip并且没有配置外网ip
		...
		if err != nil {
			mslog.Panic("get external ip failed", zap.Error(err))
		}
	}
	...
	default:
		mslog.Panic("cluster.role must be proxy or replica, but got " + role)
	}
...
func (o *Options) configureAuth() {
	...
	if strings.TrimSpace(o.Jwt.Secret) == "" {
		...
		if !msutil.FileExists(secretFile) {
	...
	for _, authUserStr := range authUsers {
		...
		if strings.Contains(authUserStr, "[") && strings.Contains(authUserStr, "]") {
			...
			if len(match) > 0 {
				...
				if len(userStrs) < 2 {
					mslog.Panic("auth user format error", zap.String("authUserStr", authUserStr))
				}
				username := userStrs[0]

				if username == o.ManagerUID {
					mslog.Panic("auth user username can not be manager", zap.String("username", username))
				}
			...
			if userStrs[2] != string(resource.All) {
				mslog.Panic("auth user permission format error", zap.String("authUserStr", authUserStr))
			}
	...
	if err != nil {
		mslog.Panic("create snowflake node failed", zap.Error(err))
	}
	o.messageIdGen = node
...
func getIntranetIP() string {
	intranetIPs, err := msutil.GetIntranetIP()
	if err != nil {
		panic(err)
	}
...
func GetExternalIP() (string, error) {
	...
	if err != nil {
		if !os.IsNotExist(err) {
			mslog.Warn("read external_ip.txt error", zap.Error(err))
		}
	} else if len(externalIPBytes) > 0 {
		return string(externalIPBytes), nil
	}

	externalIP, err := msutil.GetExternalIP()
	if err != nil {
		return "", err
	}
	if externalIP != "" {
