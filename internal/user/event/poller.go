...
func newPoller(index int, eventPool *EventPool) *poller {
	...
	p := &poller{
		...
		index:     index,
		eventPool: eventPool,
		Log:       mslog.NewMSLog(fmt.Sprintf("poller[%d]", index)),
		contextPool: &sync.Pool{
			New: func() interface{} {
...
func (p *poller) connsByDeviceFlag(uid string, deviceFlag msproto.DeviceFlag) []*eventbus.Conn {
	h := p.handler(uid)
	if h == nil {
		return nil
	}
...
func (p *poller) connCountByDeviceFlag(uid string, deviceFlag msproto.DeviceFlag) int {
	h := p.handler(uid)
	if h == nil {
		return 0
	}
