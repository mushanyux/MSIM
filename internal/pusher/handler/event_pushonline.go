...
func (h *Handler) processChannelPush(events []*eventbus.Event) {
	// fakeChannelId, channelType := msutil.ChannelFromlKey(channelKey)
	for _, e := range events {
		...
		for _, toConn := range toConns {
			...
			if options.G.Logger.TraceOn && e.ChannelType == msproto.ChannelTypePerson { // 暂时只打印个人频道的推送，因为群的话日志会太多
				h.Trace("推送在线消息...",
					"pushOnline",
			...
			recvPacket.Framer = msproto.Framer{
			...
			recvPacket.ChannelID = sendPacket.ChannelID
			if recvPacket.ChannelType == msproto.ChannelTypePerson &&
...
func encryptMessagePayload(payload []byte, conn *eventbus.Conn) ([]byte, error) {
	...
	// 加密payload
	payloadEnc, err := msutil.AesEncryptPkcs7Base64(payload, aesKey, aesIV)
	if err != nil {
		return nil, err
	}
...
func makeMsgKey(signStr string, conn *eventbus.Conn) (string, error) {
	aesKey, aesIV := conn.AesKey, conn.AesIV
	// 生成MsgKey
	msgKeyBytes, err := msutil.AesEncryptPkcs7Base64([]byte(signStr), []byte(aesKey), []byte(aesIV))
	if err != nil {
		mslog.Error("生成MsgKey失败！", zap.Error(err))
		return "", err
	}
	return msutil.MD5(string(msgKeyBytes)), nil
}
