...
import (
	...
	"github.com/mushanyux/MSIM/pkg/grpcpool"
	"github.com/mushanyux/MSIM/pkg/msdb"
	"github.com/mushanyux/MSIM/pkg/mshook"
...
func New() *Webhook {
	eventPool, err := ants.NewPool(options.G.EventPoolSize, ants.WithPanicHandler(func(err interface{}) {
		mslog.Panic("Webhook panic", zap.Any("err", err), zap.Stack("stack"))
	}))
	if err != nil {
		panic(err)
	}
	...
	backend := diskqueue.New(
		...
		func(lvl diskqueue.LogLevel, f string, args ...interface{}) {
			mslog.Info("webhook backend", zap.String("lvl", lvl.String()), zap.String("f", f), zap.Any("args", args))
		},
	)

	return &Webhook{
		Log:              mslog.NewMSLog("Webhook"),
		eventPool:        eventPool,
...
// Online 用户设备上线通知
func (w *Webhook) Online(uid string, deviceFlag msproto.DeviceFlag, connId int64, deviceOnlineCount int, totalOnlineCount int) {
...
func (w *Webhook) Offline(uid string, deviceFlag msproto.DeviceFlag, connId int64, deviceOnlineCount int, totalOnlineCount int) {
	w.onlinestatusLock.Lock()
	defer w.onlinestatusLock.Unlock()
	online := 0
...
func (w *Webhook) notifyOfflineMsg(e *eventbus.Event, subscribers []string) {
	...
	if options.G.Channel.SubscriberCompressOfCount > 0 && len(subscribers) > options.G.Channel.SubscriberCompressOfCount {
		...
		defer gWriter.Close()
		_, err := gWriter.Write([]byte(msutil.ToJSON(subscribers)))
		if err != nil {
			w.Error("压缩订阅者失败！", zap.Error(err))
		} else {
			toUIDs = make([]string, 0)
			compress = "gzip"
			compressToUIDs = buff.Bytes()
		}
	}
		...
		Data: types.MessageOfflineNotify{
			MessageResp: types.MessageResp{
				Header: types.MessageHeader{
					RedDot:    msutil.BoolToInt(sendPacket.RedDot),
					SyncOnce:  msutil.BoolToInt(sendPacket.SyncOnce),
					NoPersist: msutil.BoolToInt(sendPacket.NoPersist),
				},
			...
			ToUids:          toUIDs,
			Compress:        compress,
			CompresssToUids: compressToUIDs,
			SourceId:        int64(options.G.Cluster.NodeId),
		},
	})
}

func (w *Webhook) AppendMessageOfNotifyQueue(messages []msdb.Message) error {
	for _, msg := range messages {
...
func (w *Webhook) notifyQueueLoop() {
	...
	localMessageCache := make([]msdb.Message, 0, options.G.Webhook.MsgNotifyEventCountPerPush*2)
	...
	for {
		...
		case data, ok := <-w.backend.ReadChan():
			...
			var msg msdb.Message // 假设 msdb.Message 是一个结构体。如果它是指针类型，应为 *msdb.Message
		...
		if shouldPush && len(localMessageCache) > 0 {
			...
			messagesToPush := make([]msdb.Message, len(localMessageCache))
...
func (w *Webhook) pushMessages(messages []msdb.Message, errMessageIDMap map[int64]int) (allSucceeded bool, retryableMessages []msdb.Message) {
	if len(messages) == 0 {
		return true, nil
	}
	...
	if sendErr != nil {
		...
		retryableMessages = make([]msdb.Message, 0)
...
func (w *Webhook) sendWebhookForGRPC(event string, data []byte) error {
	...
	// 	return err
	// }
	cli := mshook.NewWebhookServiceClient(clientConn)

	sendCtx, sendCancel := context.WithTimeout(context.Background(), time.Second*10)
	defer sendCancel()
	resp, err := cli.SendWebhook(sendCtx, &mshook.EventReq{
		Event: event,
		Data:  data,
	})
	...
	if resp.Status != mshook.EventStatus_Success {
